...
const ParticleCard = ({
  ...
  const clearAllParticles = useCallback(() => {
    timeoutsRef.current.forEach(clearTimeout as unknown as (id: number) => void);
    timeoutsRef.current = [];
...
const MagicBento = ({
  ...
  const itemsToRender: ProjectItem[] = (items && items.length ? items : cardData.map(c => ({
    title: c.title,
  ...
  const itemsCropped = itemsToRender.slice(0, Math.max(0, itemsToRender.length - 2));
        ...
        {itemsCropped.map((card, index) => {
          ...
          const cardProps = {
            ...
            style: {
              ...
              ['--glow-color']: glowColor as string
          ...
          if (enableStars) {
            return (
              ...
              >
                {!!card.image && (
                  <img className="card__image-preview" src={typeof card.image === 'string' ? card.image : undefined} alt="" />
                )}
                <div className="card__header">
                  <div className="card__label">{card.label || 'Project'}</div>
                </div>
